...
class Recorder:
    ...
    def set_recordings_folder(self, folder):
        self.recordings_folder = folder

    # Keyboard events recording
    def record_keyboard(self, rec_folder, mouse_listener, screen_recorder):
    ...
    def record_mouse(self, rec_folder):
    ...
    def init_screen_recorder(self, rec_folder):
    ...
    def record_screen(self, rec_folder, screen_recorder):
    ...
    def init_recording_infos(self, rec_folder, rec_title):
        recording_infos_file = open(rec_folder + 'recording_infos.txt', 'w')
        recording_infos_file.write('title|' + rec_title + '\n')
    ...
    # Recorder main recording function
    def record(self, rec_title):
        recording_id = str(time.time_ns())
        rec_folder = self.recordings_folder + recording_id + '/'
        ...
        self.init_recording_infos(rec_folder, rec_title)
        mouse_listener = self.record_mouse(rec_folder)
        screen_recorder = self.init_screen_recorder(rec_folder)
        self.record_keyboard(rec_folder, mouse_listener, screen_recorder)
        self.record_screen(rec_folder, screen_recorder)
        shutil.make_archive(self.recordings_folder + recording_id, 'zip', rec_folder)
